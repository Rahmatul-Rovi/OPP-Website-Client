...
<div className="lg:col-span-2 bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
  <div className="flex justify-between items-center mb-8">
    <div>
      <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Sales Performance</h2>
      <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Last 7 Days Analysis</p>
    </div>
    <div className="flex gap-6">
      <div className="flex items-center gap-2">
        <span className="w-3 h-3 rounded-full bg-indigo-600 shadow-[0_0_10px_rgba(99,102,241,0.3)]"></span>
        <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Revenue</span>
      </div>
      <div className="flex items-center gap-2">
        <span className="w-3 h-3 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.3)]"></span>
        <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Orders</span>
      </div>
    </div>
  </div>
  ...
  <div className="h-[400px] w-full">
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={stats.graphData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }} barGap={8}>
        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
        
        {/* X-Axis: Din-er naam sposto korlam */}
        <XAxis 
          dataKey="name" 
          axisLine={false} 
          tickLine={false} 
        ...
        {/* 🔥 CUSTOM TOOLTIP: Mouse nile eita bhashbe */}
        <Tooltip 
                ...
                <div className="bg-white p-4 shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-[24px] border border-slate-50 min-w-[180px]">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="text-sm font-black text-slate-900 font-mono">৳{payload[0].value}</span>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-cyan-400"></div>
                        <span className="text-[11px] font-bold text-slate-500 uppercase">Orders</span>
                      </div>
                      <span className="text-sm font-black text-slate-900">{payload[1]?.value || 0}</span>
                    </div>
                  </div>
                </div>
              );
...
<button 
  onClick={() => {
    // 🛡️ Check: Jodi sale data ba cart na thake
    if (!sale || !sale.cart) {
      Swal.fire({
        title: 'Error!',
